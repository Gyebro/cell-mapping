...
#pragma omp parallel for
        ...
        virtual void generateImage(std::string path, SCMColoringMethod<CellType, IDType>* coloringMethod, bool emphasize,
                IDType x0, IDType y0, IDType xw, IDType yw) = 0;
    ...
    template <class CellType, class IDType, class StateVectorType>
    class BSCMJpeg : public BSCM<CellType, IDType, StateVectorType> {
    public:
        BSCMJpeg(StateVectorType center, StateVectorType width, const std::vector<IDType>& cellCounts,
             DynamicalSystemBase<StateVectorType> *systemPointer) : BSCM<CellType, IDType, StateVectorType>(center, width, cellCounts, systemPointer) {

        }
        void generateImage(std::string path, SCMColoringMethod<CellType, IDType> *coloringMethod=nullptr, bool emphasize=false,
            IDType x0=0, IDType y0=0, IDType xw=0, IDType yw=0) override {
            ...
            for (IDType b = 0; b<this->css.getBlockCount(); b++) {
                std::string blockfilepath = path+"block_"+std::to_string(b)+".jpg";
                ...
                if (xw == 0) xw = this->css.getBlock(b).getCellCounts()[0];
                if (yw == 0) yw = this->css.getBlock(b).getCellCounts()[1];
                ...
                for (size_t i=0; i<yw; i++) {
                    for (size_t j=0; j<xw; j++) {
                        ...
                        id = this->css.getBlock(b).getIDFromCellCoord(cellCoord);
                        CellType cell = this->css.getBlock(b).getCell(id);
                        std::vector<char> rgb = coloringMethod->createColor(cell, this->periodicGroups);
    ...
    using SCMB32 = BSCMJpeg<SCMCell<uint32_t>, uint32_t, StateVectorType>;

    template <class StateVectorType>
    using SCMB64 = BSCMJpeg<SCMCell<uint64_t>, uint64_t, StateVectorType>;

    template <class CellType, class IDType, class StateVectorType>
    class BSCMQt : public BSCM<CellType, IDType, StateVectorType> {
    protected:
    ...
    public:
        ...
        size_t addImageBlock(StateVectorType center, StateVectorType width, const std::vector<IDType>& cellCounts) {
            blockResults->push_back(QImage(cellCounts[0], cellCounts[1], QImage::Format_RGB32));
            return this->addBlock(center, width, cellCounts);
        }
        ...
        void generateImage(std::string path, SCMColoringMethod<CellType, IDType> *coloringMethod=nullptr, bool emphasize=false,
            ...
            // Generate multiple images corresponding to each block
            for (IDType b = 0; b<this->css.getBlockCount(); b++) {
                // TODO: Only update block if it has changed!
                //std::cout << "Generating QImage: " << b << std::endl;
                size_t xw = this->css.getBlock(b).getCellCounts()[0];
                size_t yw = this->css.getBlock(b).getCellCounts()[1];
