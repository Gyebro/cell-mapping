...
int main() {
    ...
    BSCM<SCMCell<uint32_t>, uint32_t, vec2> scm1(center, width, cells, &system);
    scm1.solve(max_steps);
    vec2 center2 = center + vec2({width[0], 0});
    scm1.addBlock(center2, width, cells);
    scm1.generateImage("cb_v2_pre_update.jpg");
    scm1.update(max_steps);
    scm1.generateImage("cb_v2_post_update.jpg");
    scm1.reSolve(max_steps);
    ...
    BSCM<SCMCell<uint32_t>, uint32_t, vec2> scm3(center3, width3, cells3, &system);
    scm3.solve(20);
